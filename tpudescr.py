...
def run_command_return_all(*args, xtrace=True):
    xtrace and print("+", *args, file=stderr, flush=True)
    return json.loads(run([*args, "--format=json"], check=True, capture_output=True).stdout.decode().strip())

def tpucmd(*args):
    return "gcloud", "compute", "tpus", *args

def qrcmd_cmn(cmd):
    return tpucmd("queued-resources", cmd)
...
def main(argv):
    ...
    try:
        ...
        ep = run_command(*tpucmd("tpu-vm", "describe", FLAGS.node_id, f"--zone={tpu_zone}"), key="networkEndpoints")
        print(ep)
        print(f"number of workers is {len(json.loads(ep))}")
        exit()
